...
rule sort_queryname_normal:
    ...
    conda:
        "envs/somatic.yaml"
    shell:
        "gatk SortSam --java-options -Xmx56G -I {input.normalBam} -O {output} -SO queryname"
...
rule mark_adapters_normal:
    ...
    conda:
        "envs/somatic.yaml"
    shell:
        "gatk MarkIlluminaAdapters -I {input} -O {output.bam} -M {output.metrics}"
...
rule sort_queryname_tumor:
    ...
    conda:
        "envs/somatic.yaml"
    shell:
        "gatk SortSam --java-options -Xmx56G -I {input.tumorBam} -O {output} -SO queryname"
